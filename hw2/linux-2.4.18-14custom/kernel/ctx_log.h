...
#define CTX_TASK_PRIO_ARRAY(p) \
    ...
    ? CTX_EXPIRED \
    : ((p)->array == task_rq(p)->short_array \
    ? CTX_SHORTS \
    : ((p)->array == task_rq(p)->overdue_array \
    ? CTX_OVERDUE_SHORTS \
    : CTX_OTHER_RQ )))))
...
inline static void set_ctx_log_rec_task_info(ctx_log_rec_task_info_t* info, task_t* p) {
    ...
    info->remaining_cooloff_cycles = p->cooloffs_left;
    info->initial_cooloff_cycles = p->requested_cooloffs;
    info->requested_time = p->requested_time_ms;
    info->timeslice_for_next_cooloff = p->next_requested_time;
...
inline static void log_ctx(task_t *prev, task_t *next) {
    ...
    if (rq) {       // to avoid disaster if assumptions do not take place.
        ...
        assign_best_prio_queue(record->best_prio_queues+2, rq->short_array);
        assign_best_prio_queue(record->best_prio_queues+3, rq->overdue_array);
        
        record->nr_active[0] = rq->active->nr_active;
        record->nr_active[1] = rq->expired->nr_active;
        record->nr_active[2] = rq->short_array->nr_active;
        record->nr_active[3] = rq->overdue_array->nr_active;
    }
